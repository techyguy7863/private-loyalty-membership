 ...
 */
export const CONTRACT_ADDRESS = "0200a49f71c3e82d60b5e9148f3c72b8109d64e52187f394c8b61e0594a2b7c1";
...
export type PLMWitnesses = Witnesses<MemberPrivateState>;

export function createPrivateWitnesses(state: MemberPrivateState): PLMWitnesses {
  return {
    memberSecretKey: (context: any) => [state, state.memberSecretKey],
    loyaltyProofNonce: (context: any) => [state, state.loyaltyProofNonce],
    membershipRecordHash: (context: any) => [state, state.membershipRecordHash]
  };
}

export class PrivateLoyaltyMembershipClient {
  private contract: Contract<MemberPrivateState>;
  private privateState: MemberPrivateState;
  private walletConnected: boolean = false;
  private walletAddress: string = '';

  constructor(initialState?: Partial<MemberPrivateState>) {
    ...
    this.privateState = {
      memberSecretKey: initialState?.memberSecretKey || new Uint8Array(32).fill(1),
      loyaltyProofNonce: initialState?.loyaltyProofNonce || new Uint8Array(32).fill(2),
      membershipRecordHash: initialState?.membershipRecordHash || new Uint8Array(32).fill(3)
    };
  ...
  public getContract(): Contract<MemberPrivateState> {
